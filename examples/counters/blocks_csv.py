...
title = text(
    font_name="Times-Roman", font_size=8, x=1.4, y=0.9, align="centre",
    text=T('{{TITLE}}'))
move = text(
    font_name="Times-Roman", font_size=12, x=0.65, y=2.3,
    text=T('{{MOVE}}'))
strength = text(
    font_name="Times-Roman", font_size=12, x=2.1, y=2.3,
    text=T('{{STRENGTH}}'))
outline = rectangle(
    x=0.45, y=0.45, width=2.0, height=2.0,
    stroke_width=1, stroke=T('{{BORDER}}'), fill=None)
pic = image(T('images/{{IMAGE}}'), x=0.7, y=0.95, width=1.5, height=1.1)

# outer "dots" templates
top4 = sequence(
     square(
         x=0.9, y=0.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 4),
     interval_x=0.29)
right3 = sequence(
     square(
...
low2 = sequence(
     square(
         x=1.2, y=2.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 2),
     interval_x=0.29)
left1 = square(
...
top3 = sequence(
     square(
         x=1.05, y=0.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}')),
     setting=(1, 3),
     interval_x=0.29)
right2 = sequence(
     square(
     ...
     interval_y=0.29)
low1 = square(
    x=1.3, y=2.35, side=0.25, stroke=lbrown, stroke_width=1, fill=T('{{BORDER}}'))
dots3 = group(top3, right2, low1)

# shields
shield = Common(x=1.1, y=1.1, height=0.9, width=0.7, hatch_count=1, hatch_width=3)
