...
as its ``x`` & ``y`` or "top" and "left" positions, a ``width`` and a ``height``,
the ability to be rotated, and the addition of text in form of a ``label``,
``heading`` or ``title``.

There is no such shape as a "default" Image; but one can be quickly inserted
by simply providing the correct path and filename of the image you want to
display:

.. code:: python

    Image("my-image-filename.png")


.. NOTE::

   There is more detail about the many properties that can be defined for a
   Image in the :ref:`customised Image <imageIndex>` section.
