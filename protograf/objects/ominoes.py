...
class PolyominoObject(RectangleShape):
    ...
    def __init__(self, _object=None, canvas=None, **kwargs):
        ...
        if not correct:
            _sets = "; ".join(issue)
            feedback(f"Problem with polyomino settings: {_sets}.", True)
        # tetris
    ...
    def validate_properties(self):
        """Ensure all PolyominoObject settings are correct."""
        correct = True
        issue = []
    ...
    def set_tetris_style(self) -> tuple:
        """Get colors and set centre-shape for Tetris Tetronimo.

        Returns:
            tuple: color of outer (top) and inner (bottom) areas
        """
        match self.letter:
            case "i" | "I":  # aqua
                t3dcolors = Tetris3D(
            ...
            for col, number in enumerate(item):
                ...
                if number != 0:
                    ...
                    if self.tetris and self.is_tetronimo:
                        color_top, color_btm = self.set_tetris_style()
...
class PentominoObject(PolyominoObject):
    ...
    def draw(self, cnv=None, off_x=0, off_y=0, ID=None, **kwargs):
        ...
        super().draw(cnv=cnv, off_x=off_x, off_y=off_y, ID=ID, **kwargs)
